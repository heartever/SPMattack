...
//	int bitarray[512];
//	int bitarraysl = 0;
  printf("_gcry_mpi_ec_mul_point: %p\n", &_gcry_mpi_ec_mul_point);
  printf("_gcry_mpi_ec_dup_point: %p\n", &_gcry_mpi_ec_dup_point);
//  printf("_gcry_mpi_ec_add_points: %p\n", &_gcry_mpi_ec_add_points);
//  printf("_gcry_free: %p\n", &_gcry_free);
//  printf("_gcry_mpi_test_bit: %p\n", &_gcry_mpi_test_bit);
//  printf("add_points_edwards: %p\n", &add_points_edwards);
//  printf("ec_pow2: %p\n", &ec_pow2);
      ...
      if (mpi_is_secure (scalar))
      ...
      else
        {
          for (j=nbits-1; j >= 0; j--)
            {
              _gcry_mpi_ec_dup_point (result, result, ctx);
          //    printf("%d: %d\n", j, mpi_test_bit (scalar, j));
              if (mpi_test_bit (scalar, j))
            //  int tmp = mpi_test_bit (scalar, j);
             //  printf("%d: %d\n", j, tmp);
             // bitarray[bitarraysl++] = tmp;
             // mpi_test_bit (scalar, j);
              // if(tmp) 
                	_gcry_mpi_ec_add_points (result, result, point, ctx);
            }
        }
      //int m;
      //for(m = 0; m < bitarraysl; m++)
      //  printf("%d\n", bitarray[m]);
      return;
    }
